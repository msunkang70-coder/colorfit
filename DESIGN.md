...
## Change Summary — 오리지널(v1~v3) → 현재(v4) 변경 사항

> 초기 디자인 시스템(2026-03-28)에서 현재(2026-04-13)까지 변경된 내용 요약

### 테마 전환 (가장 큰 변경)
| 항목 | v1~v3 (라이트) | v4 (현재, 다크 글래스모피즘) |
|------|--------------|--------------------------|
| 배경 | `#F8F6F3` (Warm Off-White) | `#1A1714` (웜 차콜) + 룩북 이미지 + 오버레이 |
| 카드 | `#F0EDE8` (Beige) 불투명 | `rgba(255,255,255,0.08)` 반투명 + blur 12px |
| 텍스트 | `#222222` (Charcoal) | `rgba(240,237,232,0.9)` |
| CTA | `#964F4C` 단색 | `linear-gradient(#7A3E3C → #964F4C → #B5605D)` + 글로우 |
| 칩/태그 | `#F0EDE8` bg + border | `rgba(255,255,255,0.06)` bg + blur |
| 전체 무드 | 따뜻한 잡지 느낌 | iOS 다크 + 패션 룩북 |

### 타이포그래피 변경
| 항목 | 원본 | 현재 | 이유 |
|------|------|------|------|
| h1 font-size | 36px | **32px** | 모바일 화면에서 가독성 |
| h1 line-height | 1.15 | **1.2** | 줄간 여유 확보 |
| Pretendard 로딩 | Google Fonts CDN | **시스템 fallback** | CDN 로딩 제거, 성능 개선 |

### 컬러 값 변경
| 축 | 원본 Light | 현재 Light | 원본 Dark | 현재 Dark |
|----|-----------|-----------|----------|----------|
| color (ch) | #A07830 | **#DDB67D** | #C49540 | **#E8C693** |
| style (sf) | #6B5B8A | **#6B5876** | #8971A6 | **#896E96** |
| tpo (pcf) | 변경 없음 | #964F4C | 변경 없음 | #C4726F |
| fit (of) | 변경 없음 | #4F97A3 | 변경 없음 | #6BADB9 |

### 구조 추가 (v1~v3에 없던 것)
| 항목 | 설명 |
|------|------|
| **배경 이미지 시스템** | 7화면 × 남녀 분기 + 다크 오버레이 그래디언트 |
| **글래스모피즘 컴포넌트** | glass-card, glass-cta, glass-chip 3종 |
| **프로덕션/데모 모드** | NEXT_PUBLIC_DEMO_MODE 환경변수 분기 |
| **설문 바텀시트** | 다크 blur 20px + Quick Survey 디자인 |
| **CSS 변수 매핑** | v1 약어(pcf/of/ch/sf) ↔ v2 축 이름(tpo/fit/color/style) |

### 컴포넌트 변경
| 컴포넌트 | 원본 | 현재 |
|---------|------|------|
| 설문 팝업 | 라이트 바텀시트, 숫자 버튼 | **다크 blur, 이모지, 시맨틱 컬러** |
| 상품 CTA | "상품 보러가기" → 직접 링크 | **"유사 상품 찾기" → 네이버 쇼핑 검색** |
| 레이아웃 | 항상 iPhone 프레임 | **프로덕션: 풀스크린 / 데모: 프레임** |
